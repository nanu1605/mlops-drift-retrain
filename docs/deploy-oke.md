...
A1 is arm64; the local image was x86 — rebuild multi-arch and push:
```bash
...
docker buildx build --platform linux/arm64 \
  ...
  -t docker.io/<you>/mlops-drift:arm64 --push .
```

## 3. Point the overlay + values at your stuff
- `deploy/k8s/oke/kustomization.yaml` → `images[0].newName: docker.io/<you>/mlops-drift`.
